...
| Rendering/data | Async React Server Components by default; all public routes use `export const dynamic = "force-dynamic"`; all CMS traffic goes through `fetchCms()` in `lib/api.ts` — `no-store`, 8 s timeout, one retry — plus approved static fallback data |
...
└─ Vercel → Next.js App Router (`frontend/src/app/**/page.tsx`)
   ...
   │  └─ `lib/api.ts`
   │     ├─ NEXT_PUBLIC_API_URL
   │     ├─ fetchCms: no-store, 8 s timeout, one retry
...
- **Next routes:** `src/app/<route>/page.tsx`; dynamic segments `[slug]`/`[focus]`; define metadata where useful. Public content is force-dynamic and stays that way. Rendering region is pinned to `sin1` in `frontend/vercel.json`, beside the lab backend and the audience — that alone puts every page near 250 ms. Do not cache these pages with `revalidate`: Vercel prerenders a cached page during the build, from a US container that cannot reach the lab backend reliably, so each deploy ships a landing page with its news bands missing until the first revalidation.
