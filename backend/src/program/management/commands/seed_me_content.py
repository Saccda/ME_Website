...
#
# Offsets are days from the seed run rather than fixed dates, so a freshly seeded
# site always has events that are genuinely still to come. These are placeholders
# for authors to rewrite in Wagtail, so unlike everything else in this file they
# are created with get_or_create — re-running the seed must never overwrite an
# edit someone has made.
NEWS_ITEMS = [
    (
    ...
    def handle(self, *args, **options):
        ...
        for index, (title, offset, excerpt, focus_code) in enumerate(
            ...
            NewsEvent.objects.get_or_create(
                ...
                defaults={
                    ...
                    "title": title,
                    "excerpt": excerpt,
                    "image": focus_objects[focus_code].image
                    if focus_code
                    else hero_image,
        ...
        self.stdout.write(
            self.style.SUCCESS(
                ...
                "research, focus details, partners, facilities, media, and "
                "starter news and events."
            )
        )
